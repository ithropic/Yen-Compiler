...
class TypeChecker implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  public Void visitIfStmt(Stmt.If stmt) {
    ...
    if (stmt.condition.type != Type.BOOL) {
      error(stmt.keyword, "If condition must evaluate to a boolean.");
    }
  ...
  public Void visitWhileStmt(Stmt.While stmt) {
    check(stmt.condition);
    if (stmt.condition.type != Type.BOOL) {
      error(stmt.keyword, "While condition must evaluate to a boolean.");
    }
    check(stmt.body);
    return null;
  }
  ...
  public Void visitVarStmt(Stmt.Var stmt) {
    if (stmt.initializer != null) {
      ...
      if (!isAssignable(stmt.initializer.type, stmt.type)) {
        error(stmt.name, "Variable initialized with an incompatible type.");
      }
    }
